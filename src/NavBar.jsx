...
function NavBar({ onSearch }) {
  ...
  const handleLogin = (values, setUser) => {
    const { email, password } = values;
    const user = usersData.users.find(
      (user) => user.email === email && user.password === password
    );
  ...
  const handleSignup = (values, setUser) => {
    const { firstName, lastName, email, password } = values;
    const newUser = {
      id: usersData.users.length + 1,
      firstName,
      ...
      <Modal isOpen={loginIsOpen} onClose={() => setLoginIsOpen(false)}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <Formik
              initialValues={{ email: "", password: "" }}
              onSubmit={(values) => handleLogin(values, setUser)}
            >
              {({ isSubmitting }) => (
                <Form>
                  <Field type="email" name="email" placeholder="Email" />
        ...
        <ModalContent>
          ...
          <ModalBody>
            <Formik
              ...
              onSubmit={(values) => handleSignup(values, setUser)}
            >
              {({ isSubmitting }) => (
                <Form>
          ...
          <ModalFooter>{"Welcome! Lets get tipsy!"}</ModalFooter>
        </ModalContent>
      </Modal>
    </>
  );
